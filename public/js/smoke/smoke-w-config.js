...
let config = window.smoke_config || {
...
let count = 0;
let colorArr = window.smoke_color || [Math.random() + 0.2, Math.random() + 0.2, Math.random() + 0.2];
canvas.addEventListener("mousemove", function (e) {
  count++;
  if (window.smoke_color) {
  colorArr = window.smoke_color;
  } else if (count > 25) {
